...
- 新增 run 外循环核心：RunState/DeckState/RunManager，迁移 round 生成与难度曲线、收益计算与阶段推进逻辑。
- 新增商店、奖励、结算三类基础场景与脚本（run_shop/run_reward/run_end），用于跑通回合循环。
- 新增商店条目生成、购买与升级（3 合 1 链路）逻辑，并支持 reroll/freeze 基础行为。
- 新增金币计算与计入逻辑（材质与兔子加成），与回合结算收入合并。
...
- 当前 UI 为功能性版本，主要用于验证回合链路，后续可替换为完整视觉版。
- 难度暂时默认 easy，可在开始菜单扩展难度选择入口。
